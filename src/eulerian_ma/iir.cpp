...
#include "iir.h"
 ...
 /**********************************************************************
   ...
   function as a pointer to an array of iir_float_ts of length 2n.
   ...
   function as a pointer to an array of iir_float_ts of length 2n. Storage
   ...
   p  -  Pointer to an array of iir_float_ts where p[2i] (i=0...n-1) is
...
iir_float_t *binomial_mult(int n, iir_float_t *p)
{
	...
	a = (iir_float_t *)calloc(2 * n, sizeof(iir_float_t));
...
/**********************************************************************
  ...
  to the function as a pointers to arrays of iir_float_ts of length 2n. The real
  ...
  a pointer to an array of iir_float_ts of length 4n. The real and imaginary
  ...
  b  -  Pointer to an array of iir_float_ts of length 2n.
  c  -  Pointer to an array of iir_float_ts of length 2n.
*/
...
iir_float_t *trinomial_mult(int n, iir_float_t *b, iir_float_t *c)
{
	...
	a = (iir_float_t *)calloc(4 * n, sizeof(iir_float_t));
	if (a == NULL) return(NULL);
...
/**********************************************************************
  ...
  filter. The coefficients are returned as an array of iir_float_ts.
...
iir_float_t *dcof_bwlp(int n, iir_float_t fcf)
{
	...
	iir_float_t theta;     // M_PI * fcf / 2.0
	iir_float_t st;        // sine of theta
	iir_float_t ct;        // cosine of theta
	iir_float_t parg;      // pole angle
	iir_float_t sparg;     // sine of the pole angle
	iir_float_t cparg;     // cosine of the pole angle
	iir_float_t a;         // workspace variable
	iir_float_t *rcof;     // binomial coefficients
	iir_float_t *dcof;     // dk coefficients

	rcof = (iir_float_t *)calloc(2 * n, sizeof(iir_float_t));
	if (rcof == NULL) return(NULL);

	theta = static_cast<iir_float_t>(M_PI) * fcf;
	st = sin(theta);
	ct = cos(theta);

	for (k = 0; k < n; ++k)
	{
		parg = static_cast<iir_float_t>(M_PI) * (iir_float_t)(2 * k + 1) / (iir_float_t)(2 * n);
		sparg = sin(parg);
		cparg = cos(parg);
		a = static_cast<iir_float_t>(1.0) + st * sparg;
		rcof[2 * k] = -ct / a;
...
{
	int k;            // loop variables
	iir_float_t theta;     // M_PI * (f2f - f1f) / 2.0
	iir_float_t cp;        // cosine of phi
	iir_float_t st;        // sine of theta
	iir_float_t ct;        // cosine of theta
	iir_float_t s2t;       // sine of 2*theta
	iir_float_t c2t;       // cosine 0f 2*theta
	iir_float_t *rcof;     // z^-2 coefficients
	iir_float_t *tcof;     // z^-1 coefficients
	iir_float_t *dcof;     // dk coefficients
	iir_float_t parg;      // pole angle
	iir_float_t sparg;     // sine of pole angle
	iir_float_t cparg;     // cosine of pole angle
	iir_float_t a;         // workspace variables

	cp = cos(static_cast<iir_float_t>(M_PI) * (f2f + f1f) / static_cast<iir_float_t>(2.0));
	theta = static_cast<iir_float_t>(M_PI) * (f2f - f1f) / static_cast<iir_float_t>(2.0);
	st = sin(theta);
	ct = cos(theta);
	s2t = static_cast<iir_float_t>(2.0) * st * ct;        // sine of 2*theta
	c2t = static_cast<iir_float_t>(2.0) * ct * ct - static_cast<iir_float_t>(1.0);  // cosine of 2*theta

	rcof = (iir_float_t *)calloc(2 * n, sizeof(iir_float_t));
	tcof = (iir_float_t *)calloc(2 * n, sizeof(iir_float_t));
	...
	for (k = 0; k < n; ++k)
	{
		...
		sparg = sin(parg);
		cparg = cos(parg);
		a = static_cast<iir_float_t>(1.0) + s2t * sparg;
		rcof[2 * k] = c2t / a;
		rcof[2 * k + 1] = s2t * cparg / a;
		tcof[2 * k] = static_cast<iir_float_t>(-2.0)*cp*(ct + st * sparg) / a;
		tcof[2 * k + 1] = static_cast<iir_float_t>(-2.0)*cp*st*cparg / a;
	}
...
iir_float_t *dcof_bwbs(int n, iir_float_t f1f, iir_float_t f2f)
{
	...
	ct = cos(theta);
	s2t = static_cast<iir_float_t>(2.0)*st*ct;        // sine of 2*theta
	c2t = static_cast<iir_float_t>(2.0)*ct*ct - static_cast<iir_float_t>(1.0);  // cosine 0f 2*theta
	...
	dcof[1] = dcof[0];
	dcof[0] = static_cast<iir_float_t>(1.0);
	for (k = 3; k <= 2 * n; ++k)
		dcof[k] = dcof[2 * k - 2];
	return(dcof);
}
...
iir_float_t *ccof_bwbs(int n, iir_float_t f1f, iir_float_t f2f)
{
	...
	alpha = static_cast<iir_float_t>(-2.0) *
		cos(static_cast<iir_float_t>(M_PI) * (f2f + f1f) / static_cast<iir_float_t>(2.0)) /
		cos(static_cast<iir_float_t>(M_PI) * (f2f - f1f) / static_cast<iir_float_t>(2.0));

	ccof = (iir_float_t *)calloc(2 * n + 1, sizeof(iir_float_t));

	ccof[0] = static_cast<iir_float_t>(1.0);

	ccof[2] = static_cast<iir_float_t>(1.0);
	ccof[1] = alpha;
	...
	for (i = 1; i < n; ++i)
	{
		...
		ccof[2] += alpha * ccof[1] + static_cast<iir_float_t>(1.0);
		ccof[1] += alpha;
	}
...
iir_float_t sf_bwlp(int n, iir_float_t fcf)
{
	...
	iir_float_t omega;     // M_PI * fcf
	iir_float_t fomega;    // function of omega
	iir_float_t parg0;     // zeroth pole angle
	iir_float_t sf;        // scaling factor

	omega = static_cast<iir_float_t>(M_PI) * fcf;
	fomega = sin(omega);
	parg0 = static_cast<iir_float_t>(M_PI) / (iir_float_t)(2 * n);

	m = n / 2;
	sf = static_cast<iir_float_t>(1.0);
	for (k = 0; k < n / 2; ++k)
		sf *= static_cast<iir_float_t>(1.0) + fomega * sin((iir_float_t)(2 * k + 1)*parg0);

	fomega = sin(omega / static_cast<iir_float_t>(2.0));

	if (n % 2) sf *= fomega + cos(omega / static_cast<iir_float_t>(2.0));
	sf = pow(fomega, n) / sf;
...
iir_float_t sf_bwhp(int n, iir_float_t fcf)
{
	...
	fomega = cos(omega / static_cast<iir_float_t>(2.0));

	if (n % 2) sf *= fomega + sin(omega / static_cast<iir_float_t>(2.0));
	sf = pow(fomega, n) / sf;
...
{
	int k;            // loop variables
	iir_float_t ctt;       // cotangent of theta
	iir_float_t sfr, sfi;  // real and imaginary parts of the scaling factor
	iir_float_t parg;      // pole angle
	iir_float_t sparg;     // sine of pole angle
	iir_float_t cparg;     // cosine of pole angle
	iir_float_t a, b, c;   // workspace variables

	ctt = static_cast<iir_float_t>(1.0) / tan(static_cast<iir_float_t>(M_PI) * (f2f - f1f) / static_cast<iir_float_t>(2.0));
	sfr = static_cast<iir_float_t>(1.0);
	sfi = static_cast<iir_float_t>(0.0);
	...
	return(static_cast<iir_float_t>(1.0) / sfr);
}
...
{
	int k;            // loop variables
	iir_float_t tt;        // tangent of theta
	iir_float_t sfr, sfi;  // real and imaginary parts of the scaling factor
	iir_float_t parg;      // pole angle
	iir_float_t sparg;     // sine of pole angle
	iir_float_t cparg;     // cosine of pole angle
	...
	tt = tan(static_cast<iir_float_t>(M_PI) * (f2f - f1f) / static_cast<iir_float_t>(2.0));
	sfr = static_cast<iir_float_t>(1.0);
	sfi = static_cast<iir_float_t>(0.0);
