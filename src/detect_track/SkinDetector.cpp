...
/// \return
///
cv::Mat SkinDetector::Detect(cv::Mat image, bool drawResults, bool saveResult, int frameInd)
{
    ...
    if (m_model)
    {
		...
		cv::dilate(m_skinMask, m_skinMask, cv::getStructuringElement(cv::MORPH_RECT, cv::Size(3, 3)));
		if (drawResults)
		{
...
bool SkinDetector::LearnModel(std::string dataPath)
{
    ...
    {
        trainColors.push_back(static_cast<float>(line[0]));
        trainColors.push_back(static_cast<float>(line[1]));
        trainColors.push_back(static_cast<float>(line[2]));
        trainMarkers.push_back(line[3] - 1);
    }
    dataFile.close();

    // Prepare data
    cv::Mat samples = cv::Mat(trainColors).reshape(1, static_cast<int>(trainMarkers.size()));
